...
export async function loadMssqlSettings(): Promise<MssqlSettings | null> {
    ...
    const merged: MssqlSettings = {
        ...
        attendanceQuery: env.attendanceQuery || fileSettings?.attendanceQuery || DEFAULT_ATTENDANCE_QUERY,
...
export async function saveMssqlSettings(input: {
    ...
    const next: MssqlSettings = {
        ...
        attendanceQuery: (input.attendanceQuery ?? current?.attendanceQuery ?? DEFAULT_ATTENDANCE_QUERY).trim(),
